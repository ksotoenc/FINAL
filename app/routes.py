...
def register():
    ...
    if form.validate_on_submit():
        #hashing the password given by the user to succesfully create their account, adding it to the database
        hashed_password = bcrypt.generate_password_hash(form.password.data).decode('utf-8')
        user = User(username=form.username.data, email=form.email.data, password=hashed_password)
        db.session.add(user)
        db.session.commit()
        flash(f'Account created, you are able to login!', 'success')
...
#route to a specific uses' list of posts
